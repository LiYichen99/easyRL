...
class GobangN5Net(nn.Module):

    def __init__(self):
        super(GobangN5Net, self).__init__()
        self.model = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels=16, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_features=16*5*5, out_features=25)
        )
...
if __name__ == '__main__':
    env = WrapperGoBang(board_size=5)
    eval_model = GobangN5Net()
    target_model = GobangN5Net()
